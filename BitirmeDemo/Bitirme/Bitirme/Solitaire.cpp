...
bool isOpen(int x, int y, int z)                                          // görünüt mü deðil mi
{
	for (int i = -1; i <= 1; i++)
		for (int j = -1; j <= 1; j++)
...
Solitaire::Solitaire()
{
	...
	for (int y = 0; y<18; y++)
		for (int x = 0; x<30; x++)
		{
			...
			int n = a - '0';
			for (int z = 0; z<n; z++)
				if (f(x - 1, y - 1, z)) f(x - 1, y, z) = f(x, y - 1, z) = 0;               // text sayýsý kadar döner boþ ise yerleþtirir
				else f(x, y, z) = 1;
		}
		...
		while (app.pollEvent(e))
		{
			...
			if (e.type == Event::MouseButtonReleased)
				if (e.key.code == Mouse::Right)
				{
					int n = moves.size();
					if (n == 0) continue;
					f(moves[n - 1]) *= -1; moves.pop_back();                                // hamleler geri alýnýr
